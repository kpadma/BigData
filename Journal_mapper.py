...
for line in stdin:
	...
	auth_lst = auth_tmp.split("::")
	# remove hyphen between words
	tmp[1] = tmp[1].replace('-','')
	# remove extra spaces and convert to lower case words
	words = re.findall(r'\w+', tmp[1].strip().lower())
	# Iterate over author list and words after removing stopwords to generate mapper output <author-filename:1..>
	for aut in auth_lst:
		for word in words:
			if word not in allStopWords.keys():
				print("%s-%s:1" %(aut,word))
